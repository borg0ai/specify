# Roadmap

| RFC | Title | Status |
|-----|-------|--------|
| 0001 | [Package skill payload under specify/, keep tests at repo root](rfc/completed/0001-package-skill-payload-under-specify-dir.md) | Implemented |
| 0002 | [CLI status/title hardening (Umbrella)](rfc/completed/0002-harden-status-replace-and-title-sanitize.md) | Implemented |
| 0003 | [Anchor cmdAdvance ROADMAP status replace (child of 0002)](rfc/completed/0003-anchor-advance-roadmap-status-replace.md) | Implemented |
| 0004 | [Sanitize cmdDeliver title for markdown safety (child of 0002)](rfc/completed/0004-sanitize-deliver-title-for-markdown.md) | Implemented |
| 0005 | [Tighten cmdValidate ROADMAP status probe (child of 0002)](rfc/completed/0005-tighten-validate-roadmap-status-probe.md) | Implemented |
| 0006 | [One-concern RFCs and umbrella/child convention in specify (Umbrella)](rfc/completed/0006-one-concern-and-umbrella-rfc-convention.md) | Implemented |
| 0007 | [Document one-concern and umbrella lifecycle in SKILL.md (child of 0006)](rfc/completed/0007-document-one-concern-umbrella-in-skill.md) | Implemented |
| 0008 | [Add deliver --umbrella/--parent and templates (child of 0006)](rfc/completed/0008-deliver-umbrella-parent-cli.md) | Implemented |
| 0009 | [Validate umbrella/child link consistency (child of 0006)](rfc/completed/0009-validate-umbrella-child-consistency.md) | Implemented |
| 0010 | [Finish-path sync for archive links and task board (Umbrella)](rfc/completed/0010-finish-path-sync.md) | Implemented |
| 0011 | [Rewrite markdown links when archive moves an RFC (child of 0010)](rfc/completed/0011-rewrite-links-on-archive.md) | Implemented |
| 0012 | [Fail sync-check when an RFC markdown link does not resolve (child of 0010)](rfc/completed/0012-sync-check-broken-rfc-links.md) | Implemented |
| 0013 | [Nest child tasks on deliver and check them on archive (child of 0010)](rfc/completed/0013-task-board-on-deliver-archive.md) | Implemented |
| 0014 | [Document installed-skill CLI path vs dev checkout in SKILL.md](rfc/completed/0014-document-installed-cli-path.md) | Implemented |
| 0015 | [SKILL.md must not hardcode an install path for the CLI](rfc/completed/0015-no-hardcoded-install-path.md) | Implemented |
| 0016 | [Add approval-gated implementation workflow to SKILL.md](rfc/completed/0016-approval-gated-implementation.md) | Implemented |
